...
interface IncidentReport {
  ...
  status: "PENDING" | "EN_ROUTE" | "RESOLVED" | "OPEN";
  reason?: string;
  createdAt: string;
  proofImage?: string;
...
export default function DriverIncidents() {
  ...
  // Fetch Incidents Tracker List
  const fetchIncidents = async () => {
    setLoadingIncidents(true);
    try {
      const res = await api.get("/incidents");
      if (res.data?.success) {
        setReportedIncidents(res.data.data || []);
      }
    } catch {
      // Use initial state or fallback
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await api.post("/incidents/driver", payload);
      } catch {
      ...
      showToast("success", `Incident "${selectedPreset.title}" reported successfully!`);
      setDetails("");
      setVoiceRecorded(false);
      setVoiceSeconds(0);
      void fetchIncidents();
  ...
  // Format Status Badge for Tracker
  const renderStatusBadge = (status: string) => {
    switch (status?.toUpperCase()) {
      case "RESOLVED":
        return (
          <span className="flex items-center gap-1 rounded-full border border-emerald-300 bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700">
      ...
      case "IN_PROGRESS":
        return (
          <span className="flex items-center gap-1 rounded-full border border-sky-300 bg-sky-50 px-2.5 py-1 text-[11px] font-bold text-sky-700">
      ...
      default:
        return (
          <span className="flex items-center gap-1 rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-[11px] font-bold text-amber-700">
  ...
  const filteredIncidents = reportedIncidents.filter((item) => {
    if (activeFilterTab === "PENDING") return item.status !== "RESOLVED";
    if (activeFilterTab === "RESOLVED") return item.status === "RESOLVED";
    return true;
  });
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <header className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.3em] text-emerald-600">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Rapid incident reporting with live GPS telemetry & voice memo support.
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          {/* ── LEFT COLUMN: PRESETS + FORM (8 COLS) ── */}
          <div className="lg:col-span-7 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm space-y-5">
              ...
              >
                {submitting ? (
                  <>
                    <RefreshCw className="h-5 w-5 animate-spin" /> Submitting Report...
                  </>
                ) : (
                  <>
                    <ShieldAlert className="h-5 w-5" /> Submit Ground Incident
                  </>
                )}
              </button>
            </form>
                ...
                <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1">
                  {filteredIncidents.map((incident, idx) => (
                    <motion.div
                      ...
                      className="flex items-start justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:bg-slate-100/80"
                    >
                      <div className="space-y-1 min-w-0 flex-1">
                        ...
                        <p className="text-[10px] font-semibold text-slate-400">
                          Logged at {new Date(incident.createdAt || Date.now()).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}
                        </p>
                      </div>
